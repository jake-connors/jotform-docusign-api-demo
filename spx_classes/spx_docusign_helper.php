...
class SPX_Docusign_Helper
{
    ...
    private $account_id = ""; // prod account under apps and keys //
    private $secret_key = ""; // prod account //found under apps and keys -> actions edit

     //need a new email group for spx for this, and that email has to be added in docusign and the key is attached to that account so they get notified for new apps
    public $new_cust_credit_email = ""; // prod account, new customer's credit apps
    private $new_cust_credit_user_id = ""; //tied to the email above in prod docusign

    //need a new email group for this, we dont know why the key is commented out 
    //we can use the same email as the above (new customer credit apps) and attach the top and underneath to same key and email

    private $existing_cust_credit_email = ""; 
    //private $existing_cust_credit_email = "cronordering@fwwebb.com"; // prod account, existing customer's credit apps
    // private $existing_cust_credit_user_id = "f6415c77-f585-4168-aa76-577cdceae614"; //this key is removed bc this no longer is an eamil notification that is being received 

    //need a new email group for spx, the key relates to that account 
    private $tax_exempt_email = ""; // prod account, tax exempt notify
    private $tax_exempt_user_id = ""; //user id tied to the email account above 
    ...
    {
        $this->return_url = \Globals::getSpxOrderingServerUrl() . "/docusign_api.php?mode=return"; // landing page handling
        if (!\Globals::isProd()) {
            $this->account_id = ""; // dev (demo) account
            $this->secret_key = ""; // dev (demo) account

			// $this->new_cust_credit_email = "";
            // $this->new_cust_credit_user_id = ""; //attached to jakes email
			$this->new_cust_credit_email = "";
			$this->new_cust_credit_user_id = ""; //userid for creonbteam // in demo docusign 

			// $this->existing_cust_credit_email = "";
            // $this->existing_cust_credit_user_id = ""; //attached to jakec but tied to karlas account? 
            $this->existing_cust_credit_email = "";
            $this->existing_cust_credit_user_id = ""; //userid for creonbteam // in demo docusign
            
            // $this->tax_exempt_email = "";
            // $this->tax_exempt_user_id = ""; //for jake
			$this->tax_exempt_email = "";
            $this->tax_exempt_user_id = ""; //user id for crtaxexempt //in demo docusign 

            // $this->inbox_folder_id = ""; // user specific: jake
            // $this->sent_folder_id = ""; // user specific: jake

            $this->inbox_folder_id = ""; //folders on creonbteam spx (created folders)
            $this->sent_folder_id = ""; //folder on creonbteam spx (created folders)
			// $this->inbox_folder_id = ""; 
            // $this->sent_folder_id = "";
        }
        if ($auth_code == "") {
    ...
    public function createEnvelopeNew($args)
    {
        ...
        if ($type === "cust_credit_app_new" || $type === "cust_credit_app_existing") {
            $content_bytes = file_get_contents($_SERVER["DOCUMENT_ROOT"] . "/spx_templates/spx_credit_application.pdf");
        } else if ($type === "tax_exempt") {
            $content_bytes = file_get_contents($_SERVER["DOCUMENT_ROOT"] . "/spx_templates/spx_upload_tax_exempt.pdf");
        } else if ($type === "file_upload") {
            $content_bytes = file_get_contents($_SERVER["DOCUMENT_ROOT"] . "/spx_templates/spx_upload_files.pdf");
        } else if ($type === "pg_app") {
            $content_bytes = file_get_contents($_SERVER["DOCUMENT_ROOT"] . "/spx_templates/spx_pg_application.pdf");
            $document_id = 999;
        }
        ...
        if ($type === "cust_credit_app_new" || $type === "cust_credit_app_existing") {
            ...
            // uploading files (file upload program from ERP -> BP W4DOCUSIGNUPLOAD)
            // setting $args properties for the recipient view
            ...
            $args["unique_id"] = $args["client_user_id"]; // setting for recipient view
    ...
    public function prepareAndSendApprovalStage($env_id, $rec_role, $cust_num, $cust_name, $has_exempt_attachment, $branch)
    {
        $br_num = \Erp_Helper::getCustBranchNum($cust_num); 
        //$br_num = "125"; 
        ...
        $email_body_extra = "Cust#: " . $cust_num . "<br>Branch: " . $br_num . "<br>Name: " . $cust_name;
        $dup_customers = [];
        $dup_customers = $this->getDupCustomers($env_id);
        if ($has_exempt_attachment || count($dup_customers) > 0) {
            $email_body_extra .= "<h3 style='text-align: left'>Notes:</h3><div style='border: 2px solid black; padding: 15px; text-align: left'>";
        }
        if ($has_exempt_attachment) {
            $email_body_extra .= "<h4 style='margin-top: 0px; margin-bottom: 0px;'>This customer has uploaded a tax exempt form to their application. Please find the file within their application.</h4>";
        }
        if (count($dup_customers) > 0) {
            if ($has_exempt_attachment) $email_body_extra .= "<br>";
            if (count($dup_customers) == 1) {
                $email_body_extra .= "<b>Another Stockpile Express customer was found with a matching value:</b>";
            } else {
                $email_body_extra .= "<b>Other Stockpile Express customers were found with matching values:</b>";
            }
            $count = 1;
            foreach ($dup_customers as $dup) {
                $email_body_extra .= "<br><span>&nbsp;&nbsp;&nbsp;(" . $count . ") " . $dup["type"] . " matches with: Cust # <b>" . $dup["num"] . " - " . $dup["name"] . "</b></span>";
                $count++;
            }
        }
        if ($has_exempt_attachment || count($dup_customers) > 0) {
            $email_body_extra .= "</div>";
        }
        ...
        if (!$has_sent_approval_stage) {
            ...
            \SPX_Email_Helper::sendApprovalStage($env_id, $credit_dept_email, $credit_dept_name, $email_subject, $dup_customers, $br_num, $cust_num, $cust_name, $email_body_extra, $is_2nd_approval_stage);
            // \Email_Helper::sendApprovalStage($env_id, $credit_dept_email, $credit_dept_name, $email_subject, $email_body_extra, $is_2nd_approval_stage);
    ...
    {
        $envelope_api = $this->_getEnvelopeApi();
        $pg_base64 = base64_encode(file_get_contents($_SERVER["DOCUMENT_ROOT"] . "/spx_templates/spx_pg_application.pdf"));
        ...
        // send cust email to get into the pg app
        require_once $_SERVER["DOCUMENT_ROOT"] . "/spx_classes/spx_email_helper.php";
        \SPX_Email_Helper::sendPG($env_id, $orig_signer_email, $orig_signer_name);
    }
    ...
    public function addFilesToCreditApp($env_id, $files)
    {
        ...
        $documents = [];
        $docs_count = count($all_env_docs) + 1;
    ...
    public function send2ndApprovalStage($env_id, $cust_num, $cust_name, $branch, $has_exempt_attachment)
    {
        $br_num = \Erp_Helper::getCustBranchNum($cust_num);
        //$br_num="125"; 
        // get 1st recpient to see if coming from jotform (new cust) or coming from fwwebb (existing cust)
        $recipient = $this->getEnvelopeRecipient($env_id, 1);
        ...
        $email_body_extra = "Cust#: " . $cust_num . "<br>Branch: " . $branch . "<br>Name: " . $cust_name;
        $email_subject = "Credit App | Cust#: " . $cust_num . ", BR: " . $branch . ", Name: " . $cust_name;
        $email_body_extra .= "<h3 style='text-align: left'>Notes:</h3><div style='border: 2px solid black; padding: 15px; text-align: left'>";
        $email_body_extra .= "<h4 style='margin-top: 0px; margin-bottom: 0px;'>This customer has submitted a Signature of Personal Guarantee.</h4>";
        if ($has_exempt_attachment) {
            $email_body_extra .= "<h4 style='margin-top: 0px; margin-bottom: 0px;'>This customer has uploaded a tax exempt form to their application. Please find the file within their application.</h4>";
        }
        if (count($dup_customers) > 0) {
    ...
    public function getEnvelopeFormData($env_id)
    {
        ...
        $resp = curl_exec($curl);
        curl_close($curl);
        $json_decoded = json_decode($resp, true)["formData"];
        $ret = [];
    ...
    public function getEnvelopeType($env_id)
    {
        ...
        foreach ($all_recipients as $recipient) {
            ...
            if ($roleName === "Credit Applicant New" || $roleName === "Credit Applicant Existing" || $roleName === "Credit Approver") {
                $env_type = "credit_app";
            } elseif ($roleName === "Credit Applicant 2" || $roleName === "Credit Approver 2") {
                $env_type = "credit_app_2nd_approval_stage";
            } elseif ($roleName === "Exempt Uploader") {
                $env_type = "tax_exempt";
            } elseif ($roleName === "File Uploader") {
                $env_type = "file_upload";
            } elseif ($roleName === "PG Signer") {
                $env_type = "pg_app";
            }
        }
        return $env_type;
    ...
    private function _getTabs($type, $recipient_id, $pre)
    {
        ...
        if ($type === "cust_credit_app_new" || $type === "cust_credit_app_existing") {
            if ($recipient_id == 1) {
                $tabs_json = file_get_contents($_SERVER["DOCUMENT_ROOT"] . "/spx_templates/spx_tabs_for_customer.json"); //FOR PROD
                // $tabs_json = file_get_contents($_SERVER["DOCUMENT_ROOT"] . "/spx_templates/dev_spx_tabs_for_customer.json"); //FOR DEV
            } elseif ($recipient_id == 2) {
                $tabs_json = file_get_contents($_SERVER["DOCUMENT_ROOT"] . "/spx_templates/spx_tabs_for_credit.json"); //FOR PROD
                // $tabs_json = file_get_contents($_SERVER["DOCUMENT_ROOT"] . "/spx_templates/dev_spx_tabs_for_credit.json"); //FOR DEV
            }
        } elseif ($type === "tax_exempt") {
            $tabs_json = file_get_contents($_SERVER["DOCUMENT_ROOT"] . "/spx_templates/spx_tabs_for_upload_tax_exempt.json"); //FOR PROD
            // $tabs_json = file_get_contents($_SERVER["DOCUMENT_ROOT"] . "/spx_templates/dev_spx_tabs_for_upload_tax_exempt.json"); //FOR TEST
        } elseif ($type === "file_upload") {
            $tabs_json = file_get_contents($_SERVER["DOCUMENT_ROOT"] . "/spx_templates/spx_tabs_for_upload_files.json"); //FOR PROD
            // $tabs_json = file_get_contents($_SERVER["DOCUMENT_ROOT"] . "/spx_templates/dev_spx_tabs_for_upload_files.json"); //FOR TEST
        } elseif ($type === "pg_app") {
            $tabs_json = file_get_contents($_SERVER["DOCUMENT_ROOT"] . "/spx_templates/spx_tabs_for_pg.json"); //FOR PROD
            // $tabs_json = file_get_contents($_SERVER["DOCUMENT_ROOT"] . "/spx_templates/dev_spx_tabs_for_pg.json"); //FOR TEST
        } elseif ($type === "credit_app_add_pg") {
            if ($recipient_id == 3) {
                $tabs_json = file_get_contents($_SERVER["DOCUMENT_ROOT"] . "/spx_templates/spx_tabs_for_pg.json"); //FOR PROD
                // $tabs_json = file_get_contents($_SERVER["DOCUMENT_ROOT"] . "/spx_templates/dev_spx_tabs_for_pg.json"); //FOR TEST 
            } elseif ($recipient_id == 4) {
                // $tabs_json = file_get_contents($_SERVER["DOCUMENT_ROOT"] . "/spx_templates/dev_spx_tabs_for_2nd_approval_stage.json"); //FOR test
                $tabs_json = file_get_contents($_SERVER["DOCUMENT_ROOT"] . "/spx_templates/spx_tabs_for_credit_2nd_approval_stage.json"); //FOR PROD 
            }
        }
        $tabs_json = json_decode($tabs_json, true);
        $ret_tabs = [];
        foreach ($tabs_json as $tab_group => $tabs) {
            // regex to turn keys to snake_case
            $tab_group_name = strtolower(preg_replace(['/([a-z\d])([A-Z])/', '/([^_])([A-Z][a-z])/'], '$1_$2', $tab_group));
            $temp_tabs = [];
            foreach ($tabs as $tab) {
                $temp_t = [];
                $found_prefill = false;
     ...
     * Validate 'auth_code' by instatiating SPX_
     *Docusign_Helper with $auth_code and $access_token is returned if $auth_code is valid
     * @return string
     */
    public function getAuthURI($state)
    {
        // the callback url will do: header("Location: {$callback_url}");
        $callback_url = \Globals::getSpxOrderingServerUrl() . "/docusign_api.php";
        $config = new \DocuSign\eSign\Configuration();
        $config->setHost($this->base_path);
        $config->addDefaultHeader('Authorization', 'Bearer ' . $this->access_token);
        $oauth = new \DocuSign\eSign\Client\Auth\OAuth();
    ...
    private function _createJWT()
    {
        ...
        // docuSign provides the private key
        $fp = fopen("/var/www/secure_keys/docusign_rsa.pem", "r");
        $private_key = fread($fp, 8192);
        fclose($fp);

        $rsa_private_key = openssl_get_privatekey($private_key);
        // $signature parameter is also a return value
        $signature_boolean = openssl_sign("$header_encoded.$payload_encoded", $signature, $rsa_private_key, 'sha256WithRSAEncryption');
