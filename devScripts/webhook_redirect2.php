...
/*          *** NOTES ***  
    ...
    - if the envelope created is a stockpile express envelope, the customer will be redirected to the "spx_docusign_webhook" file 

    Recipients :
    - New Customer (coming from jotform)
    - Credit Dept Manager (approve/deny any credit app)
    - Existing Customer (applying for terms coming from fwwebb.com)
    ...
    - Tax Exempt (separate from regi form, coming from jotform or fwwebb.com)
...
require_once $_SERVER["DOCUMENT_ROOT"] . "/classes/crm_helper.php";
require_once $_SERVER["DOCUMENT_ROOT"] . "/classes/docusign_helper.php";
require_once $_SERVER["DOCUMENT_ROOT"] . "/classes/email_helper.php";
...
if ($is_spx) {
    ...
    $docusign_helper = new Docusign_Helper();
    ...
    if ($event === "envelope-completed") {
        if ($env_type === "tax_exempt") {
            ...
            \Email_Helper::sendCompletedPG($env_id, $docusign_helper->new_cust_credit_email, $cust_info);
        }
    } elseif ($event === "recipient-finish-later") {
        $customer_num = $docusign_helper->getCustomerNumber($env_id);
        $br_num = \Erp_Helper::getCustBranchNum($customer_num);
        ...
        if (in_array($recipient_type, $valid_finish_later_types)) {
            ...
            \Email_Helper::sendFinishLater($to, $to_name, $env_id, $rec_id, $is_session_timeout, $is_file_upload, $customer_num, $br_num);
        }
        ...
        if ($rec_role === "Credit Applicant New" || $rec_role === "Credit Applicant Existing") {
            ...
            $docusign_helper->send2ndApprovalStage($env_id, $cust_num, $cust_name, $has_exempt_attachment, $br_num, );
        ...
        if ($env_type === "credit_app" || $env_type === "credit_app_2nd_approval_stage") {
            ...
            if ($rec_id == 2 || $rec_id == 4) {
                ...
                $br_num = \Erp_Helper::getCustBranchNum($cust_num);
                $branch_record = \Erp_Helper::getBranchRecord($br_num);
                ...
                $branchRecord = \Erp_Helper::getBranchRecord($br_num);
                ...
                if ($isMotherBranch) {
                    ...
                    $motherBranchRecord = \Erp_Helper::getBranchRecord($branchMotherBr);
                ...
                if (trim($decline_reason) != "" && $to != "") {
                    ...
                    \Email_Helper::sendAccountDenied($to, $to_name, $cust_num, $decline_reason, $address1, $address2, $br_phone, $br_manager_emails);
                } else {
                    // sends email to branch managers
                    $to = $br_manager_emails;
                    \Email_Helper::sendAccountDenied($to, $to_name, $cust_num, $decline_reason, $address1, $address2, $br_phone, []);
                }
            }
        }
    ...
    function handleEnvelopeCompleted($env_id, $is_2nd_approval_stage)
    {
        ...
        $bus_phone = \Erp_Helper::phoneConvert($env_data["Business Phone"]);
        $bus_fax = $env_data["Business Fax"];
        $bus_cell = \Erp_Helper::phoneConvert($env_data["Business Cell"]);
        ...
        $credit_approve_result = \Erp_Helper::W4INTSUBCUSTCREDITAPPROVE($data_in);
        ...
        if ($credit_approve_result["primary_contact"]["found"] == "1") {
            ...
            \Crm_Helper::updateContact($cust_num, true, strtoupper($contact_first), strtoupper($contact_last), $email, "SALES CONTACT", $company_phone, $cell, $fax, $addr_line1, $addr_line2, $city, $state, $zip);
        }

        // name/label of this field changed, handle old forms that were created before the change
        if (isset($env_data["Owner 1 Cell"])) {
            $owner_1_cell = \Erp_Helper::phoneConvert($env_data["Owner 1 Cell"]);
        } else if (isset($env_data["Owner 1 Phone"])) {
            $owner_1_cell = \Erp_Helper::phoneConvert($env_data["Owner 1 Phone"]);
        } else {
            $owner_1_cell = "";
        }
        ...
        if ($credit_approve_result["found_owner1_contact"] == "1") {
            \Crm_Helper::updateContact($cust_num, false, $owner1_first, $owner1_last, $own_con_email, $own_con_title, $own_con_phone);
        } else {
            \Crm_Helper::createContactFromCreditApp($cust_num, $owner1_first, $owner1_last, $own_con_email, $own_con_title, $own_con_phone);
        }

        if ($owner2_first != "" && $owner2_last != "") {
            if ($credit_approve_result["found_owner2_contact"] == "1") {
                \Crm_Helper::updateContact($cust_num, false, $owner2_first, $owner2_last, $own2_con_email, $own2_con_title, $own2_con_phone);
            } else {
                \Crm_Helper::createContactFromCreditApp($cust_num, $owner2_first, $owner2_last, $own2_con_email, $own2_con_title, $own2_con_phone);
            }
        }
        ...
        \Crm_Helper::updateProfile($cust_num, [], $env_data["Business URL"], $suppliers);
        $br_num = \Erp_Helper::getCustBranchNum($cust_num);
        $branch_record = \Erp_Helper::getBranchRecord($br_num);
        ...
        \Email_Helper::sendAccountApproved($send_email_to, $send_email_to_name, $branch_name_and_state, $branch_phone, $credit_amount, $cust_num, $notify_using_einvoice_email, $notify_using_estatement_email, $br_addr1, $br_addr2);
        \Email_Helper::sendBranchAccountApproved($branch_email, $branch_and_name, $cust_num, strtoupper($bus_name));
    }
