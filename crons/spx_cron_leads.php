...
$save_customer_today = \Erp::getSPXSaveCustomerRecords(false)["records"];
$save_customer_yesterday = \Erp::getSPXSaveCustomerRecords(true)["records"];

$date = date("Y-m-d", strtotime("-2 days"));
$date_time_check = $date . " 19:58:00";
$registered_custs = dbi_query("SELECT * FROM cust_reg_log WHERE assigned_br = '125' AND date_time >= ?:date_time_check", ["date_time_check" => $date_time_check]);
...
foreach ($save_customer_today as $cust1) {
    ...
    foreach ($registered_custs as $cust2) {
        if (trim(strtolower($cust1["email"])) === trim(strtolower($cust2["email"]))) {
            ...
            $found_submit = true;
            \Erp::deleteSPXSaveCustomerRecord($record_key);
        }
    }
    if (!$found_submit) {
        ...
        \Erp::writeSPXSaveCustomerRecord($record_key, "1");
    }
}
...
$event = "spx-cron-leads";
$data_to_log = ["today_save_customer" => $save_customer_today, "yesterday_save_customer" => $save_customer_yesterday, "registered_custs" => $registered_custs, "new_leads_list" => $new_leads, "date_time" => $dt];
$data_to_log = json_encode($data_to_log, JSON_PRETTY_PRINT);
$params_log = [
    "event" => $event,
    "data" => $data_to_log
];
...
if (count($new_leads) > 0) {
    if (!send_email($new_leads)) {
        $sql_log = "INSERT INTO log (event, data) VALUES (?:event, ?:data)";
        $data_to_log = json_encode($new_leads, JSON_PRETTY_PRINT);
        $params_log = [
            "event" => "mail send error in cron.php",
            "data" => $data_to_log
        ];
        dbi_query($sql_log, $params_log);        
    }
...
function send_email($potential_leads) {
    ...
    if (\Globals::isProd()) {
        ...
        $mail->addBCC("jfc@fwwebb.com", ""); //change at production to johns email 
    } else {
        $mail->addAddress(\Globals::getDevEmail(), "");
    }

    $subject = "Potential Leads (from stockpileexpress.com)";
    ...
    $body .= '<tr><td style="padding:10px 24px;"><img style="border:none;" width="300" src="cid:spx_logo" /></td></tr>'; //change logo
    ...
    $body .= '<h3>Potential Leads from stockpileexpress.com!</h3>';
    if (!\Globals::isProd()) {
    ...
    $mail->addEmbeddedImage($DOCUMENT_ROOT . "/images/StockpileExpress_Logo.png", "spx_logo"); //change logo to spx
